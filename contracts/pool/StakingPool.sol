...
contract StakingPool is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function rewardPerBlock(address token) public view returns (uint256) {
        ...
        return
            reward.rewardBalance.div(
                reward.expiredBlock.sub(reward.startBlock)
            );
    }

    function _updateReward(address token) internal {
        Reward memory reward = rewards[token];
        uint256 startBlock = Math.max(
        ...
        if (endBlock > startBlock) {
            if (totalPower > 0) {
                ...
                reward.accRewardPerPower = reward.accRewardPerPower.add(
                    profit.mul(ACC_REWARD_PRECISION) / totalPower
                );
            }
    ...
    function _updateUserDebt(
        ...
        for (uint256 i = 0; i < rewardList.length; i++) {
            ...
            if (addOrSub) {
                users[user].rewardDebt[rewardList[i]] = users[user]
                    .rewardDebt[rewardList[i]]
                    .add(
                        int256(
                            power.mul(reward.accRewardPerPower) /
    ...
    function _arrayRemove(address user, uint256 tokenId) internal {
        for (uint256 i = 0; i < users[user].stakedAssets.length; i++) {
            if (tokenId == users[user].stakedAssets[i]) {
                users[user].stakedAssets[i] = users[user].stakedAssets[
                    users[user].stakedAssets.length - 1
                ];
    ...
    function _checkSlots(address user) internal {
        ...
        console.log(
            "user has %s slots, and %s used slots, and %s availableSlots.",
            // , user
        ...
        require(
            users[user].numAvailableSlots + users[user].stakedAssets.length ==
                users[user].numSlots,
            "ESLOT"
        );
    ...
    function pendingReward(address token, address _user)
        external
        view
        returns (uint256 pending)
    {
        ...
        if (endBlock > startBlock && totalPower != 0) {
            ...
            accRewardPerPower = accRewardPerPower.add(
                tokenReward.mul(ACC_REWARD_PRECISION) / totalPower
            );
        }
        pending = uint256(
            int256(user.power.mul(accRewardPerPower) / ACC_REWARD_PRECISION)
    ...
    function addReward(
        ...
        if (reward.expiredBlock > block.number) {
            reward.rewardBalance = reward
                .rewardBalance
                .mul(reward.expiredBlock.sub(block.number))
    ...
    function increaseSlot(uint256 slotTokenId) public nonReentrant {
        require(
            IERC721(slotAsset).ownerOf(slotTokenId) == msg.sender,
            "ENOTEXIST"
        );
        IERC721(slotAsset).safeTransferFrom(
            msg.sender,
            address(this),
            slotTokenId
        );
    ...
    function deposit(uint256[] calldata stakingNFTs) public nonReentrant {
        ...
        require(
            users[msg.sender].numAvailableSlots >= stakingNFTs.length,
            "ESLOT"
        );
    ...
    function withdraw(uint256[] calldata stakingNFTs) public nonReentrant {
        require(stakingNFTs.length > 0, "EARRAY");
        require(
            users[msg.sender].numSlots - users[msg.sender].numAvailableSlots >=
                uint8(stakingNFTs.length),
            "ESLOT"
        );
    ...
    function harvestAll(address to) public {
        for (uint256 i = 0; i < rewardList.length; i++) {
            ...
            int256 accReward = int256(
                user.power.mul(reward.accRewardPerPower) / ACC_REWARD_PRECISION
            );
            uint256 _pendingReward = uint256(
                accReward.sub(user.rewardDebt[token])
            );
